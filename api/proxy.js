...
function rewriteHTML(html, base){
  ...
  html = html.replace(/(href|src|action|poster|data-src|srcset)="([^"]*)"/gi,(m,attr,url)=>{
...
export default async function handler(req,res){
  try{
    ...
    if(contentType.includes("text/html")){
      ...
      // inject runtime for navigation, forms, fetch/XHR, history API
      const inject = `<script>
function encode(u){return "/proxy?url="+btoa(u);}
function resolve(u){try{return new URL(u,location.href).href}catch{return u}}

/* domains to fully proxy without rewriting internal navigation (CAPTCHA domains) */
const bypassDomains = ["www.google.com","www.gstatic.com","www.recaptcha.net","hcaptcha.com","assets.hcaptcha.com","challenges.cloudflare.com","challenges.cloudflare.net"];
...
document.addEventListener("click", e=>{
  ...
  if(!href) return;
  if(href.startsWith("javascript:") || !shouldRewrite(href)) return;
  e.preventDefault();
...
document.addEventListener("submit", e=>{
  const form = e.target;
  if(!form.action || !shouldRewrite(form.action)) return;
  e.preventDefault();
...
window.fetch = function(resource, opts){
  if(typeof resource==="string" && shouldRewrite(resource)) resource = encode(resolve(resource));
...
window.XMLHttpRequest = function(){
  ...
  xhr.open = function(method, url, ...rest){
      if(url.startsWith("http") && shouldRewrite(url)) url = encode(resolve(url));
...
/* trap history API */
const push = history.pushState; history.pushState=function(a,b,url){if(url && shouldRewrite(url)) url=encode(resolve(url)); return push.call(this,a,b,url);}
const replace = history.replaceState; history.replaceState=function(a,b,url){if(url && shouldRewrite(url)) url=encode(resolve(url)); return replace.call(this,a,b,url);}

/* trap window.location assignments */
const loc = window.location;
Object.defineProperty(window,"location",{set:function(url){if(shouldRewrite(url)) loc.href=encode(resolve(url)); else loc.href=url;}, get:function(){return loc;}});
</script>`;
